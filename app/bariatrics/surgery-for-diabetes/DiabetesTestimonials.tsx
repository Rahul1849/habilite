...
export default function DiabetesTestimonials() {
  const [selectedVideo, setSelectedVideo] = useState<string | null>(null)

  const metabolicTestimonials = getTestimonialsByTreatment('Metabolic Surgery')
  const additionalTestimonials = testimonials.filter(
    (t) =>
      !metabolicTestimonials.find((mt) => mt.id === t.id) &&
      (t.treatment.toLowerCase().includes('diabetes') ||
        t.treatment.toLowerCase().includes('metabolic') ||
        t.treatment.toLowerCase().includes('bariatric'))
  )

  const combined = [...metabolicTestimonials, ...additionalTestimonials]
  const videoTestimonials = combined.filter((t) => t.videoUrl)
  const textTestimonials = combined.filter((t) => !t.videoUrl)
    ...
    <section className="my-12">
      <div className="text-center mb-8">
        ...
        <p className="text-lg text-gray-600 max-w-2xl mx-auto">
          Hear from patients who chose metabolic surgery for diabetes reversal at Habilite Clinics
        </p>
      </div>
