...
export default function BariatricTestimonials() {
  const [selectedVideo, setSelectedVideo] = useState<string | null>(null)
  
  // Get all testimonials related to bariatric surgery
  const bariatricTestimonials = getTestimonialsByTreatment('Bariatric Surgery')
  // Also include any testimonials that mention bariatric, weight loss, or gastric
  const additionalTestimonials = testimonials.filter(
    t => !bariatricTestimonials.find(bt => bt.id === t.id) && 
    (t.treatment.toLowerCase().includes('bariatric') || 
     t.treatment.toLowerCase().includes('weight loss') ||
     t.treatment.toLowerCase().includes('gastric') ||
     t.treatment.toLowerCase().includes('sleeve') ||
     t.treatment.toLowerCase().includes('bypass'))
  )
  
  const allBariatricTestimonials = [...bariatricTestimonials, ...additionalTestimonials]
  const videoTestimonials = allBariatricTestimonials.filter(t => t.videoUrl)
  const textTestimonials = allBariatricTestimonials.filter(t => !t.videoUrl)
    ...
    <section className="my-12">
      <div className="text-center mb-8">
        ...
        <p className="text-lg text-gray-600 max-w-2xl mx-auto">
          Real stories from patients who underwent bariatric surgery with Dr. Kapil Agrawal
        </p>
      </div>
