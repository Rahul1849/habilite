    ...
    try {
      const response = await fetch('/api/send-email', {
        ...
        body: JSON.stringify({
          ...
          phone: phone.trim(),
          subject: 'WhatsApp Care Reminders - Obesity Treatment',
          message: `Request to subscribe to WhatsApp care reminders for obesity treatment. Phone: ${phone.trim()}`,
        }),
      })
      ...
      if (result.success) {
        ...
        setTimeout(() => {
          redirectToWhatsApp({
            ...
            phone: phone.trim(),
            subject: 'WhatsApp Care Reminders - Obesity Treatment',
            message: `I would like to subscribe to WhatsApp care reminders for obesity treatment.`,
          })
  ...
  return (
    <div className="rounded-3xl bg-white border border-gray-100 shadow-xl p-6 sm:p-8 flex flex-col gap-6">
      <div className="flex items-start gap-4">
        ...
        <div>
          <p className="text-sm uppercase tracking-[0.4em] font-bold text-[#0891b2]">Weekly WhatsApp Coaching</p>
          <h3 className="text-2xl font-semibold text-gray-900 mt-2">📲 Get Diet Reminders & Workout Nudges</h3>
          <p className="text-gray-600 mt-2">
            Enter your number to receive doctor-approved meal plans, hydration reminders, and accountability messages directly on WhatsApp.
          </p>
        </div>
      </div>
      ...
      {step === 'submitted' && (
        <div className="space-y-3">
          <div className="flex items-center gap-2 rounded-2xl bg-emerald-50 text-emerald-800 px-4 py-3 text-sm font-semibold">
            ...
            Thank you! Your personalised coaching plan will arrive on WhatsApp soon.
          </div>
          <button
            type="button"
            onClick={resetForm}
