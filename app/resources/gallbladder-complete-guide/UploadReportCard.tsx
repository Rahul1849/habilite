    ...
    try {
      const response = await fetch('/api/send-email', {
        ...
        body: JSON.stringify({
          ...
          name: 'Report Upload',
          phone: phone.trim(),
          subject: 'Report Upload - Gallbladder Treatment',
  ...
  return (
    <div className="rounded-3xl bg-white shadow-xl border border-gray-100 p-6 sm:p-8 flex flex-col gap-6">
      <div className="flex items-start gap-4">
        ...
        <div>
          <h3 className="text-xl font-semibold text-gray-900">Upload Your Ultrasound Report for Explanation</h3>
          <p className="text-gray-600 mt-2">
            PDF / JPG / PNG (max 5 MB). Reviewed personally by Dr. Kapil Agrawal&apos;s gallbladder team.
          </p>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-dashed border-[#0891b2]/40 bg-[#f8fbff] px-4 py-6 flex flex-col gap-4">
        <label className="inline-flex items-center justify-center gap-3 px-5 py-3 rounded-2xl border border-[#0891b2] text-[#0891b2] font-semibold cursor-pointer hover:bg-white transition-colors">
          ...
          <FileText size={18} />
          {fileName ? 'Upload another report' : 'Choose report to upload'}
        </label>
        {fileName && (
        ...
        {step === 'uploaded' && (
          <div className="flex items-center gap-2 rounded-2xl bg-slate-900 text-white px-4 py-3 text-sm font-medium shadow">
            ...
            Report received. Please verify your phone so we can update you personally.
          </div>
        )}
        {step === 'submitted' && (
          <div className="flex items-center gap-2 rounded-2xl bg-emerald-50 text-emerald-800 px-4 py-3 text-sm font-semibold">
            <CheckCircle2 size={18} />
            Thank you! Your report is queued for specialist review. Our coordinator will call you shortly.
          </div>
        )}
      </div>
      ...
      <div className="space-y-4">
        {step === 'initial' && (
          <p className="text-sm text-gray-500">Need help uploading? WhatsApp the report to +91 99994 56455.</p>
        )}
