...
export default function AppendixTestimonials() {
  const [selectedVideo, setSelectedVideo] = useState<string | null>(null)

  const appendixTestimonials = testimonials
    .filter(
      (t) =>
        t.treatment.toLowerCase().includes('appendicitis') ||
        t.treatment.toLowerCase().includes('appendix') ||
        t.tags?.some((tag) => tag.toLowerCase().includes('appendix') || tag.toLowerCase().includes('appendicitis'))
    )
    .slice(0, 6)

  // If no specific testimonials, show general laparoscopic surgery testimonials
  const displayTestimonials = appendixTestimonials.length > 0 
    ? appendixTestimonials 
    : testimonials.filter((t) => t.treatment.toLowerCase().includes('laparoscopic')).slice(0, 6)
  ...
  return (
    <section>
      <div className="text-center mb-8">
        <h2 className="text-3xl md:text-4xl font-bold mb-4 text-gray-900">Appendix Patient Testimonials</h2>
        <p className="text-lg text-gray-600 max-w-2xl mx-auto">
          Real stories from patients who underwent laparoscopic appendectomy with Dr. Kapil Agrawal
        </p>
      </div>
                ...
                <>
                  <div className="relative aspect-video w-full cursor-pointer group" onClick={() => setSelectedVideo(testimonial.videoUrl || null)}>
                    <Image
                      ...
                      alt={`${testimonial.patientName} - Appendix Testimonial`}
                      fill
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={() => setSelectedVideo(null)}>
          <div className="relative w-full max-w-4xl aspect-video" onClick={(e) => e.stopPropagation()}>
            ...
            <iframe
              ...
              title="Appendix Video Testimonial"
