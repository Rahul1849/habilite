...
export default function LipomaTestimonials() {
  const [selectedVideo, setSelectedVideo] = useState<string | null>(null)

  const lipomaTestimonials = testimonials
    .filter(
      (t) =>
        t.treatment.toLowerCase().includes('lipoma') ||
        t.tags?.some((tag) => tag.toLowerCase().includes('lipoma'))
    )
    .slice(0, 6)

  const displayTestimonials = lipomaTestimonials.length > 0
    ? lipomaTestimonials
  ...
  return (
    <section>
      <div className="text-center mb-8">
        <h2 className="text-3xl md:text-4xl font-bold mb-4 text-gray-900">Lipoma Patient Testimonials</h2>
        <p className="text-lg text-gray-600 max-w-2xl mx-auto">
          Real stories from patients who underwent lipoma treatment with Dr. Kapil Agrawal
        </p>
      </div>
                ...
                <>
                  <div className="relative aspect-video w-full cursor-pointer group" onClick={() => setSelectedVideo(testimonial.videoUrl || null)}>
                    <Image
                      ...
                      alt={`${testimonial.patientName} - Lipoma Testimonial`}
                      fill
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={() => setSelectedVideo(null)}>
          <div className="relative w-full max-w-4xl aspect-video" onClick={(e) => e.stopPropagation()}>
            ...
            <iframe
              ...
              title="Lipoma Video Testimonial"
