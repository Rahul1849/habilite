...
export default function CircumcisionTestimonials() {
  const [selectedVideo, setSelectedVideo] = useState<string | null>(null)

  const circumcisionTestimonials = testimonials
    .filter(
      (t) =>
        t.treatment.toLowerCase().includes('circumcision') ||
        t.tags?.some((tag) => tag.toLowerCase().includes('circumcision'))
    )
    .slice(0, 6)

  const displayTestimonials = circumcisionTestimonials.length > 0
  ...
  return (
    <section>
      <div className="text-center mb-8">
        <h2 className="text-3xl md:text-4xl font-bold mb-4 text-gray-900">Circumcision Patient Testimonials</h2>
        <p className="text-lg text-gray-600 max-w-2xl mx-auto">
          Real stories from patients who underwent laser or ZSR circumcision with Dr. Kapil Agrawal
        </p>
      </div>
                ...
                <>
                  <div className="relative aspect-video w-full cursor-pointer group" onClick={() => setSelectedVideo(testimonial.videoUrl || null)}>
                    <Image
                      ...
                      alt={`${testimonial.patientName} - Circumcision Testimonial`}
                      fill
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={() => setSelectedVideo(null)}>
          <div className="relative w-full max-w-4xl aspect-video" onClick={(e) => e.stopPropagation()}>
            ...
            <iframe
              ...
              title="Circumcision Video Testimonial"
