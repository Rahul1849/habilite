...
export default function GallbladderPolypsTestimonials() {
  ...
  const relatedTestimonials = testimonials
    .filter(
      (t) =>
        ...
        t.treatment.toLowerCase().includes('laparoscopic') ||
        t.treatment.toLowerCase().includes('robotic') ||
        t.treatment.toLowerCase().includes('polyp')
    )
    .slice(0, 3)

  if (relatedTestimonials.length === 0) return null

  return (
    <section className="my-12">
      <div className="text-center mb-8">
        <h2 className="text-3xl md:text-4xl font-bold mb-4 text-gray-900">Patient Testimonials</h2>
        <p className="text-lg text-gray-600 max-w-2xl mx-auto">Real stories from patients who received treatment from Dr. Kapil Agrawal</p>
      </div>
